...
import { MDLanguages, MDRatings, MDImageQuality } from "./MangaDexHelper";
...
export async function getHomepageThumbnail(stateManager: SourceStateManager) {
  return (
    (await stateManager.retrieve("homepage_thumbnail")) ?? [
...
export async function getSearchThumbnail(stateManager: SourceStateManager) {
  return (
    (await stateManager.retrieve("search_thumbnail")) ?? [
...
export async function getMangaThumbnail(stateManager: SourceStateManager) {
  return (
    (await stateManager.retrieve("manga_thumbnail")) ?? [
...
export async function getAccessToken(stateManager: SourceStateManager) {
  const accessToken: string | undefined = await stateManager.keychain.retrieve(
    "access_token"
  );
  const refreshToken: string | undefined = await stateManager.keychain.retrieve(
    "refresh_token"
  );
    ...
    form: App.createDUIForm({
      sections: async () => [
        App.createDUISection({
          ...
          id: "content",
          footer:
            "When enabled, same chapters from different scanlation group will not be shown.",
          rows: async () => {
            await Promise.all([
...
export async function accountSettings(
  ...
  if (!accessToken) {
    return App.createDUIOAuthButton({
      ...
      authorizeEndpoint:
        "https://auth.mangadex.dev/realms/mangadex/protocol/openid-connect/auth",
      ...
      responseType: {
        ...
        tokenEndpoint:
          "https://auth.mangadex.dev/realms/mangadex/protocol/openid-connect/token",
      },
            ...
            return await [
              App.createDUISelect({
                ...
                value: App.createDUIBinding({
                  ...
                  set: async (newValue) =>
                    await stateManager.store("homepage_thumbnail", newValue),
                }),
              ...
              App.createDUISelect({
                ...
                value: App.createDUIBinding({
                  ...
                  set: async (newValue) =>
                    await stateManager.store("search_thumbnail", newValue),
                }),
                allowsMultiselect: false,
              }),
              App.createDUISelect({
                ...
                value: App.createDUIBinding({
                  ...
                  set: async (newValue) =>
                    await stateManager.store("manga_thumbnail", newValue),
                }),
                allowsMultiselect: false,
              }),
            ];
