...
var _Sources = (() => {
  ...
  async function getHomepageThumbnail(stateManager) {
    return await stateManager.retrieve("homepage_thumbnail") ?? [
      MDImageQuality.getDefault("homepage")
    ];
  }
  async function getSearchThumbnail(stateManager) {
    return await stateManager.retrieve("search_thumbnail") ?? [
      MDImageQuality.getDefault("search")
    ];
  }
  async function getMangaThumbnail(stateManager) {
    return await stateManager.retrieve("manga_thumbnail") ?? [
      MDImageQuality.getDefault("manga")
    ];
  }
  async function getAccessToken(stateManager) {
    const accessToken = await stateManager.keychain.retrieve(
      "access_token"
    );
    const refreshToken = await stateManager.keychain.retrieve(
      "refresh_token"
    );
  ...
  async function parseAccessToken(accessToken) {
    ...
    const tokenBodyJSON = import_buffer.Buffer.from(tokenBodyBase64, "base64").toString(
      "ascii"
    );
  ...
  async function _authEndpointRequest(requestManager, endpoint, payload) {
    ...
    if (jsonData.result != "ok") {
      throw new Error(
        "Request failed with errors: " + jsonData.errors.map((x) => `[${x.title}]: ${x.detail}`)
      );
    }
    return jsonData;
  }
  async function accountSettings(stateManager, requestManager) {
    ...
    return App.createDUINavigationButton({
      ...
      form: App.createDUIForm({
        ...
        sections: async () => {
          const accessToken2 = await getAccessToken(stateManager);
          if (!accessToken2) {
            return [
              App.createDUISection({
                isHidden: false,
          ...
          return [
            App.createDUISection({
              ...
              id: "introspect",
              rows: async () => {
                return Object.keys(accessToken2.tokenBody).map((key) => {
                  const value = accessToken2.tokenBody[key];
              ...
              rows: async () => [
                App.createDUIButton({
                  ...
                  onTap: async () => {
                    const response = await authEndpointRequest(
                      requestManager,
                      "refresh",
                      { token: accessToken2.refreshToken }
                    );
                ...
                App.createDUIButton({
                  ...
                  onTap: async () => {
                    ...
                    await saveAccessToken(stateManager, void 0, void 0);
                  }
                })
              ]
            })
